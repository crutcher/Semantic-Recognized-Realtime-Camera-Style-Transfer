...
class VGG(nn.Module):
    def __init__(self, blocks, input_sz=224, num_classes=1000, output_stride=32):
        ...
        self.layer1 = self._build_block(
            in_channels=3, block=blocks[0], dilation=dilations[0]
        )
        self.maxpool1 = (
        ...
        self.layer2 = self._build_block(
            in_channels=blocks[0][-1], block=blocks[1], dilation=dilations[1]
        )
        self.maxpool2 = (
            nn.MaxPool2d(kernel_size=2, stride=2)
            if strides[1] == 2
        ...
        self.layer3 = self._build_block(
            in_channels=blocks[1][-1], block=blocks[2], dilation=dilations[2]
        )
        self.maxpool3 = (
            nn.MaxPool2d(kernel_size=2, stride=2)
            if strides[2] == 2
        ...
        self.layer4 = self._build_block(
            in_channels=blocks[2][-1], block=blocks[3], dilation=dilations[3]
        )
        self.maxpool4 = (
            nn.MaxPool2d(kernel_size=2, stride=2)
            if strides[3] == 2
        ...
        self.layer5 = self._build_block(
            in_channels=blocks[3][-1], block=blocks[4], dilation=dilations[4]
        )
        self.maxpool5 = (
            nn.MaxPool2d(kernel_size=2, stride=2)
            if strides[4] == 2
