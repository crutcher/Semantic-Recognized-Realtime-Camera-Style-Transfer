...
class DecoderBlock(nn.Module):
    def __init__(
        self, in_channels, out_channels, block, use_deconv=True, squeeze=1, dropout=0.2
    ):
                    ...
                    OrderedDict(
                        [
                            (
                                "conv1",
                                nn.Conv2d(
                                    in_channels, hid_channels, kernel_size=1, bias=False
                                ),
                            ),
...
class UNetPlus(BaseModel):
    def __init__(
        ...
        frozen_stages=-1,
        norm_eval=True,
        init_backbone_from_imagenet=False,
    ):
        ...
        if (
            ("resnet" in backbone)
            or ("resnext" in backbone)
            or ("wide_resnet" in backbone)
        ):
            self.backbone = getattr(backbones, backbone)(
                in_chans=in_channels, frozen_stages=frozen_stages, norm_eval=norm_eval
            )
            inplanes = 64
            block = (
                backbones.ResNetBasicBlock
                if "18" in backbone or "34" in backbone
                else backbones.ResNetBottleneckBlock
            )
            expansion = block.expansion

            self.decoder = nn.Module()
            self.decoder.layer1 = DecoderBlock(
                8 * inplanes * expansion,
                4 * inplanes * expansion,
                block,
                squeeze=squeeze,
                dropout=dropout,
                use_deconv=use_deconv,
            )
            self.decoder.layer2 = DecoderBlock(
                4 * inplanes * expansion,
                2 * inplanes * expansion,
                block,
                squeeze=squeeze,
                dropout=dropout,
                use_deconv=use_deconv,
            )
            self.decoder.layer3 = DecoderBlock(
                2 * inplanes * expansion,
                inplanes * expansion,
                block,
                squeeze=squeeze,
            ...
            self.decoder.layer4 = DecoderBlock(
                inplanes * expansion,
                inplanes,
                block,
                squeeze=squeeze,
