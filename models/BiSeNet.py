...
class SpatialPath(nn.Module):
    def __init__(self, basic_channels=64):
        ...
        self.conv2 = ConvBlock(
            ...
            out_channels=2 * basic_channels,
            kernel_size=3,
            stride=2,
            padding=1,
        )
        self.conv3 = ConvBlock(
            in_channels=2 * basic_channels,
            out_channels=4 * basic_channels,
            kernel_size=3,
            stride=2,
            padding=1,
        )
...
class BiSeNet(BaseModel):
    ...
    def forward(self, input):
        ...
        feat_os16 = F.interpolate(
            feat_os16, scale_factor=2, mode="bilinear", align_corners=True
        )
        feat_os32 = F.interpolate(
            feat_os32, scale_factor=4, mode="bilinear", align_corners=True
        )
        ...
        if self.training:
            ...
            feat_os16_sup = F.interpolate(
                feat_os16_sup, scale_factor=8, mode="bilinear", align_corners=True
            )
            feat_os32_sup = F.interpolate(
                feat_os32_sup, scale_factor=8, mode="bilinear", align_corners=True
            )
