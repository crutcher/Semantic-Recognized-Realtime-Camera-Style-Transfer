...
from timm.utils import AverageMeter, accuracy, natural_key, setup_default_logging
...
parser.add_argument(
    "--fp16", action="store_true", default=False, help="Use half precision (fp16)"
)
parser.add_argument(
    "--tf-preprocessing",
    action="store_true",
...
def validate(args):
    ...
    # create model
    model = create_model(
        args.model, num_classes=args.num_classes, in_chans=3, pretrained=args.pretrained
    )
    ...
    logging.info(
        " * Prec@1 {:.3f} ({:.3f}) Prec@5 {:.3f} ({:.3f})".format(
...
def main():
    ...
    if len(model_cfgs):
        print(
            "Running bulk validation on these pretrained models:",
            ", ".join(model_names),
        )
