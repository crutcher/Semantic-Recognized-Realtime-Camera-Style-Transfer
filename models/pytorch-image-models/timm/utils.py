...
class CheckpointSaver:
    ...
    def save_checkpoint(
        self, model, optimizer, args, epoch, model_ema=None, metric=None
    ):
        assert epoch >= 0
        ...
        if (
            ...
            if metric is not None and (
                ...
                shutil.copyfile(
                    save_path,
                    os.path.join(self.checkpoint_dir, "model_best" + self.extension),
                )
    ...
    def _save(
        self, save_path, model, optimizer, args, epoch, model_ema=None, metric=None
    ):
        save_state = {
            "epoch": epoch,
            "arch": args.model,
