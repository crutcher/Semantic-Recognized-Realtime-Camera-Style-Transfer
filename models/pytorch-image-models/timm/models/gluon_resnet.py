...
class GluonResNet(nn.Module):
    ...
    def __init__(
        ...
        if not deep_stem:
            self.conv1 = nn.Conv2d(
                in_chans, stem_width, kernel_size=7, stride=2, padding=3, bias=False
            )
        else:
            conv1_modules = [
                ...
                nn.ReLU(),
                nn.Conv2d(
                    stem_width, self.inplanes, 3, stride=1, padding=1, bias=False
                ),
            ]
...
def gluon_resnet18_v1b(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    model = GluonResNet(
        BasicBlockGl, [2, 2, 2, 2], num_classes=num_classes, in_chans=in_chans, **kwargs
    )
...
def gluon_resnet34_v1b(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    model = GluonResNet(
        BasicBlockGl, [3, 4, 6, 3], num_classes=num_classes, in_chans=in_chans, **kwargs
    )
    model.default_cfg = default_cfg
    if pretrained:
        load_pretrained(model, default_cfg, num_classes, in_chans)
...
def gluon_resnet50_v1b(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    model = GluonResNet(
        BottleneckGl, [3, 4, 6, 3], num_classes=num_classes, in_chans=in_chans, **kwargs
    )
    model.default_cfg = default_cfg
    if pretrained:
        load_pretrained(model, default_cfg, num_classes, in_chans)
