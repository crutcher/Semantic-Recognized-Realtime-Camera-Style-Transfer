...
class _BlockBuilder:
    ...
    def _make_block(self, ba):
        ...
        if bt == "ir":
            ...
            if self.verbose:
                logging.info(
                    "  InvertedResidual {}, Args: {}".format(self.block_idx, str(ba))
                )
            ...
            if self.verbose:
                logging.info(
                    "  DepthwiseSeparable {}, Args: {}".format(self.block_idx, str(ba))
                )
    ...
    def __init__(
        ...
        if not head_conv or head_conv == "none":
            ...
            self.conv_head = select_conv2d(
                in_chs, self.num_features, 1, padding=pad_type
            )
            self.bn2 = (
                None
                if self.efficient_head
                else nn.BatchNorm2d(self.num_features, **bn_args)
            )
...
def mnasnet_small(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    model = _gen_mnasnet_small(
        1.0, num_classes=num_classes, in_chans=in_chans, **kwargs
    )
    model.default_cfg = default_cfg
    if pretrained:
        load_pretrained(model, default_cfg, num_classes, in_chans)
...
def mobilenetv3_075(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    model = _gen_mobilenet_v3(
        0.75, num_classes=num_classes, in_chans=in_chans, **kwargs
    )
    model.default_cfg = default_cfg
    if pretrained:
        load_pretrained(model, default_cfg, num_classes, in_chans)
