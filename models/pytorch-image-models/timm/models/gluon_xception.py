...
class SeparableConv2d(nn.Module):
    def __init__(
        ...
        self.conv_dw = nn.Conv2d(
            ...
            kernel_size,
            stride=stride,
            padding=_pytorch_padding(kernel_size, stride, dilation),
            dilation=dilation,
            groups=inplanes,
            bias=bias,
        )
    ...
    def __init__(
        ...
        if planes != inplanes or stride != 1:
            ...
            self.skip.add_module(
                "conv1", nn.Conv2d(inplanes, planes, 1, stride=stride, bias=False)
            ),
        ...
        if grow_first:
            if start_with_relu:
                rep["act%d" % l] = nn.ReLU(
                    inplace=False
                )  # NOTE: silent failure if inplace=True here
