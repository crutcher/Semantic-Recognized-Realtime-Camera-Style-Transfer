...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        if deep_stem:
            self.conv1 = nn.Sequential(
                *[
                    ...
                    nn.Conv2d(
                        stem_width, stem_width, 3, stride=1, padding=1, bias=False
                    ),
                    norm_layer(stem_width),
                    nn.ReLU(inplace=True),
                    nn.Conv2d(
        ...
        self.layer3 = self._make_layer(
            block, 256, layers[2], stride=stride_3_4, dilation=dilation_3, **largs
        )
        self.layer4 = self._make_layer(
            block, 512, layers[3], stride=stride_3_4, dilation=dilation_4, **largs
        )
