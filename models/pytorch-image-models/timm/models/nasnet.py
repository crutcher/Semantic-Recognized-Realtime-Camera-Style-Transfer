...
class CellStem0(nn.Module):
    def __init__(self, stem_size, num_channels=42):
        ...
        self.conv_1x1.add_module(
            "conv",
            nn.Conv2d(self.stem_size, self.num_channels, 1, stride=1, bias=False),
        )
        self.conv_1x1.add_module(
            "bn",
            nn.BatchNorm2d(self.num_channels, eps=0.001, momentum=0.1, affine=True),
        )

        self.comb_iter_0_left = BranchSeparables(
            self.num_channels, self.num_channels, 5, 2, 2
        )
        self.comb_iter_0_right = BranchSeparablesStem(
            self.stem_size, self.num_channels, 7, 2, 3, bias=False
        )
        ...
        self.comb_iter_2_right = BranchSeparablesStem(
            self.stem_size, self.num_channels, 5, 2, 2, bias=False
        )
        ...
        self.comb_iter_4_left = BranchSeparables(
            self.num_channels, self.num_channels, 3, 1, 1, bias=False
        )
...
class CellStem1(nn.Module):
    def __init__(self, stem_size, num_channels):
        ...
        self.conv_1x1.add_module(
            "conv",
            nn.Conv2d(
                2 * self.num_channels, self.num_channels, 1, stride=1, bias=False
            ),
        )
        self.conv_1x1.add_module(
            "bn",
        ...
        self.final_path_bn = nn.BatchNorm2d(
            self.num_channels, eps=0.001, momentum=0.1, affine=True
        )

        self.comb_iter_0_left = BranchSeparables(
            self.num_channels, self.num_channels, 5, 2, 2, bias=False
        )
        self.comb_iter_0_right = BranchSeparables(
            self.num_channels, self.num_channels, 7, 2, 3, bias=False
        )
...
class NASNetALarge(nn.Module):
    ...
    def __init__(
        ...
        self.conv0.add_module(
            "bn", nn.BatchNorm2d(self.stem_size, eps=0.001, momentum=0.1, affine=True)
        )

        self.cell_stem_0 = CellStem0(
            self.stem_size, num_channels=channels // (channel_multiplier ** 2)
        )
        self.cell_stem_1 = CellStem1(
            self.stem_size, num_channels=channels // channel_multiplier
        )

        self.cell_0 = FirstCell(
            in_channels_left=channels,
            out_channels_left=channels // 2,
            in_channels_right=2 * channels,
            out_channels_right=channels,
        )
        self.cell_1 = NormalCell(
            in_channels_left=2 * channels,
            out_channels_left=channels,
            in_channels_right=6 * channels,
            out_channels_right=channels,
        )
        self.cell_2 = NormalCell(
            in_channels_left=6 * channels,
            out_channels_left=channels,
            in_channels_right=6 * channels,
            out_channels_right=channels,
        )
        ...
        self.cell_6 = FirstCell(
            in_channels_left=6 * channels,
            out_channels_left=channels,
            in_channels_right=8 * channels,
            out_channels_right=2 * channels,
        )
        self.cell_7 = NormalCell(
            in_channels_left=8 * channels,
            out_channels_left=2 * channels,
            in_channels_right=12 * channels,
            out_channels_right=2 * channels,
        )
        self.cell_8 = NormalCell(
            in_channels_left=12 * channels,
            out_channels_left=2 * channels,
            in_channels_right=12 * channels,
            out_channels_right=2 * channels,
        )
        ...
        self.cell_12 = FirstCell(
            in_channels_left=12 * channels,
            out_channels_left=2 * channels,
            in_channels_right=16 * channels,
            out_channels_right=4 * channels,
        )
        self.cell_13 = NormalCell(
            in_channels_left=16 * channels,
            out_channels_left=4 * channels,
            in_channels_right=24 * channels,
            out_channels_right=4 * channels,
        )
        self.cell_14 = NormalCell(
            in_channels_left=24 * channels,
            out_channels_left=4 * channels,
            in_channels_right=24 * channels,
            out_channels_right=4 * channels,
        )
