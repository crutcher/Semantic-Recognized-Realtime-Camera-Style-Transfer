...
class PlateauLRScheduler(Scheduler):
    ...
    def __init__(
        ...
        if self.warmup_updates:
            self.warmup_active = (
                warmup_updates > 0
            )  # this state updates with num_updates
            self.warmup_steps = [
                (v - warmup_lr_init) / self.warmup_updates for v in self.base_values
            ]
    ...
    def get_update_values(self, num_updates: int):
        if num_updates < self.warmup_updates:
            ...
            self.warmup_active = (
                False  # warmup cancelled by first update past warmup_update count
            )
