...
class BaseInference(object):
    ...
    def draw_background(self, image, mask):
        ...
        mask_filtered = cv2.GaussianBlur(
            mask, (self.kernel_sz, self.kernel_sz), self.sigma
        )
...
class VideoInference(BaseInference):
    def __init__(
        ...
        # Initialize
        super(VideoInference, self).__init__(
            model, color_f, color_b, kernel_sz, sigma, background_path
        )
    ...
    def predict(self, X):
        with torch.no_grad():
            if self.use_cuda:
                ...
                mask = F.interpolate(
                    mask, size=(self.H, self.W), mode="bilinear", align_corners=True
                )
