...
class BaseModel(nn.Module):
    ...
    def summary(self, input_shape, batch_size=1, device="cpu", print_flops=False):
        ...
        torchsummary.summary(
            self, input_size=input_shape, batch_size=batch_size, device=device
        )
        if print_flops:
            ...
            print(
                "Flops:  {}".format(
                    flops_to_string(counter.compute_average_flops_cost())
                )
            )
            print("----------------------------------------------------------------")
    ...
    def load_pretrained_model(self, pretrained):
        if isinstance(pretrained, str):
            print(
                "[%s] Load pretrained model from %s"
                % (self.__class__.__name__, pretrained)
            )
