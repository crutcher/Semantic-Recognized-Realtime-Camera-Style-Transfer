        ...
        if name in self.tensorboard_writer_ftns:
            ...
            try:
                ...
                raise AttributeError(
                    "type object 'WriterTensorboardX' has no attribute '{}'".format(
                        name
                    )
                )
            return attr
...
def plot_tensorboard(train_file, valid_file, scalar_names, set_grid=False):
    ...
    for scalar_name in scalar_names:
        train_lists[scalar_name] = [
            train_scalars[scalar_name][i][2] for i in range(n_epochs)
        ]
        valid_lists[scalar_name] = [
            valid_scalars[scalar_name][i][2] for i in range(n_epochs)
        ]
